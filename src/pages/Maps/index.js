...
const Maps = ({show}) => {
  ...
  const { currentUser } = useAuth(); // Menggunakan hook useAuth untuk mendapatkan currentUser
      ...
      <div className="side-detail">
        ...
        {selectedMarker ? (
          <div className="card">
            ...
            <h4>Nama Kampus: <p>{selectedMarker.namaKampus}</p></h4>
            <h4>Lokasi: <p>{selectedMarker.alamat} </p></h4>
            <h4>Status: <p>{selectedMarker.status}</p></h4>
            <h4>Akreditasi: <p>{selectedMarker.akreditas}</p></h4>
            <h4>Jumlah Fakultas: <p>{selectedMarker.jumlahFakultas}</p> </h4>
            <h4>Berdiri: <p>{selectedMarker.berdiri}</p></h4>
            <h4>Deskripsi: <p>{selectedMarker.deskripsi}</p></h4>
            {role.role === "kepala" ?(
              <div className="content-btn">
              <div>
                <span data-tooltip-id="edit-tooltip" data-tooltip-content="Edit" onClick={() => handleEditClick(selectedMarker.id)}>
                  <FaEdit className="edit-button" size={23} color="green" />
                </span>
                <Tooltip id="edit-tooltip" />
              </div>
              <div>
                <span data-tooltip-id="hapus-tooltip" data-tooltip-content="Hapus" onClick={() => handleDelete(selectedMarker.id)}>
                  <MdDeleteForever className="delete-button" size={23} color="red" />
                </span>
                  ...
                  <Popup>
                    <div className="container-popup">
                      ...
                      <div className="text-popup">
                        <label className="nama-popup">{truncateText(item.namaKampus, maxLength)}</label>
